...
namespace WebAPI.Models
{
    ...
    {
        public static DateTime _tokenGenTime = DateTime.UtcNow;
        public static AccessTokenInfo _token;        //Timeout every 10 min

        public static string _requestUri = "https://speech.platform.bing.com/synthesize";
        public static string _speechAccount = System.Configuration.ConfigurationManager.AppSettings["SPEECH_API_Name"];
        public static string _speechKey = System.Configuration.ConfigurationManager.AppSettings["SPEECH_API_Key"];

        public string Text;
        public string Lang;
        public Stream speechStream;

        public TTSpeech(string text, string lang)
        {
            this.Text = text;
            this.Lang = lang;
            ...
            try
            {
                if (_token == null || _tokenGenTime.AddMinutes(10) < DateTime.UtcNow)
                {
                    Authentication auth = new Authentication(_speechAccount, _speechKey);
                    _token = auth.GetAccessToken();
                    _tokenGenTime = DateTime.UtcNow;
                }
            }
            catch (Exception)
            {                
        ...
        public byte[] GetSpeech()
        {
            ...
            cortana.OnAudioAvailable += CatchAudio;
            ...
            if (speechStream != null)
            {
                ...
                {
                    int read;
                    while ((read = speechStream.Read(buffer, 0, buffer.Length)) > 0)
                    {
        ...
        void CatchAudio(object sender, GenericEventArgs<Stream> args)
        {
            this.speechStream = args.EventData;            
        }
